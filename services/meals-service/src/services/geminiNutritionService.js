...
class GeminiNutritionService {
  constructor() {
    ...
    console.log('Gemini service initialized with API key - using gemini-flash-latest');
  }
...
`;
  ...
  parseFoodAnalysisResponse(text) {
    try {
      console.log('\n🔍 Parsing food analysis response...');
      ...
      console.log('✅ Validation passed. Items count:', parsed.items.length);
  ...
  async analyzeNutrition(items) {
    try {
      ...
      console.error('Gemini API error in analyzeNutrition:', error);
      if (error.response) {
        console.error('Error status:', error.response.status);
        console.error('Error details:', error.response.data);
      }
